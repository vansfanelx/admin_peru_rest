    ...
    .DataTable({
        ...
                    return '<div class="text-center"><span class="label label-success">APROBADO</span></div>';
            ...
            {"data":null,"render": function ( data, type, row ) {
                ...
                +'&nbsp;<button class="btn btn-sm btn-primary btn-imprimir-egreso" data-json="'+htmlspecialchars(JSON.stringify(data))+'"><i class="fas fa-print"></i></button>'
                +'</div>';
            }}
        ],
...
// Handler para impresión robusto y seguro de egreso
$(document).on('click', '.btn-imprimir-egreso', function() {
    var data = $(this).attr('data-json');
    try {
        data = JSON.parse(data);
    } catch (e) {
        alert('Error al procesar los datos para imprimir.');
        return;
    }
    data.nombre_pc = $('#pc_name').val();
    var url = 'http://' + $('#pc_ip').val() + '/imprimir/egreso.php?data=' + encodeURIComponent(JSON.stringify(data));
    window.open(url, '_blank');
});

// Función para escapar comillas en el atributo HTML
