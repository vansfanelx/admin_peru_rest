...
$(function() {
    ...
    .on('success.form.fv', function(e) {
        ...
        if ($("#cliente_id").val() == ''){
            ...
            venta.propina = $('#propina').val() || '0.00'; // Asegura que propina siempre se envíe
            ...
            html_confirm = '<div>Se creará el documento electrónico<br> with los siguientes datos</div>';
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function (dato) {
                            ...
                            // FORZAR impresión por web cuando print_cpe = 0 (no directo)
                            // Esto aplica para boleta, factura y nota de venta
                            // Si la página está en HTTPS, nunca intentes impresión directa local (evita mixed content)
                            if($('#print_cpe').val() == 1 && location.protocol === 'http:'){
                                // imprimir directo (IP local o entorno interno, solo si la página es HTTP)
                                window.open('http://'+$('#pc_ip').val()+'/imprimir/comprobante_venta.php?id='+cod_ven,'_blank');
                                ...
                                // Visualizar automáticamente el comprobante en una nueva ventana/pestaña (Windows print, seguro para HTTPS)
                                var urlComprobante = $("#url").val()+'informe/venta_all_imp_/'+cod_ven;
                                var linkrep     = '<a class="link" onclick="window.open(\''+urlticketreparto+'\', \'_blank\');" href="javascript:void(0)"><i class="fas fa-print text-muted"></i></a>';
                                var linkrepp    = '<a class="link" onclick="window.open(\''+urlticketreparto+'\', \'_blank\');" href="javascript:void(0)"><small>Ticket reparto</small></a>';
                                var linkpdf     = '<a href="javascript:void(0)" onclick="window.open(\''+urlComprobante+'\', \'_blank\');" class="link"><i class="mdi mdi-receipt text-muted"></i></a>';
                                var linkpdff    = '<a class="link" href="javascript:void(0)" onclick="window.open(\''+urlComprobante+'\', \'_blank\');"><small>Imprimir comprobante</small></a>';
                                // Visualizar automáticamente el comprobante en una nueva ventana/pestaña (Windows print)
                                setTimeout(function(){ window.open(urlComprobante, '_blank'); }, 500);
                            }
...
var pedido = {
    ...
    refrescar: function()
    {
        ...
        this.detalle.igv      = (this.detalle.total * imp_val).toFixed(2); // IGV dinámico según configuración
...
var impPreCuenta = function(id_pedido,id_mesa,estado,wiew){
    // Log para depuración del valor pc_name antes de enviar
    console.log('impPreCuenta: pc_name =', $('#pc_name').val());
    $.ajax({
        ...
        data: {
            id_mesa: id_mesa,
            estado: estado,
            host_pc: $('#pc_name').val() // Se integra host_pc al objeto data del AJAX
        },
        success: function (r) {
            return true;
        }
        ...
        if(wiew){
            ...
            // Log para depuración de la URL de impresión y host_pc
            var url_imp = $("#url").val()+"venta/impresion_precuenta/"+id_pedido;
            console.log('impPreCuenta: url_imp =', url_imp, 'host_pc =', $('#pc_name').val());
            window.open(url_imp,'_blank');            
        }
    }); 
}
...
var printPdf = function(url, condicional) {
    if (condicional) {
        // Mostrar el PDF en una nueva ventana/pestaña para que el usuario elija la impresora
        window.open(url, '_blank');
    } else {
        // Impresión directa (como estaba actualmente)
        var Urlpdf = $("#url").val() + 'informe/venta_all_imp_/' + url;
...
// Función global para mostrar el comprobante en un modal (vista previa PDF)
function printPdfModal(url) {
    // Si ya existe el modal, lo reutiliza
    if ($('#modal-pdf-preview').length === 0) {
        $('body').append('<div class="modal fade" id="modal-pdf-preview" tabindex="-1" role="dialog" aria-labelledby="modalPdfLabel" aria-hidden="true">\
            <div class="modal-dialog modal-xl" role="document">\
                <div class="modal-content">\
                    <div class="modal-header">\
                        <h5 class="modal-title" id="modalPdfLabel">Vista previa de comprobante</h5>\
    ...
    $('#iframe-pdf-preview').attr('src', url);
...
var propina_factura = function(){
    var html_confirm = '<div>Se procederá a realizar una propina por el servicio<br><br>\
    Ingrese monto</div>\
    <form class="dec"><input class="form-control text-center w-50" type="text" id="filtro_propina" autocomplete="off" value=""/></form><br>\
    <div><span class="text-success" style="font-size: 17px;">¿Está Usted de Acuerdo?</span></div>';
    Swal.fire({
        title: 'Necesitamos de tu Confirmación',
        html: html_confirm,
        ...
        if (result.value) {
            limpiar_pagos_b();
            $('.propina').val($("#filtro_propina").val());
            $('.propina').text(formatNumber($("#filtro_propina").val()));
            calculo_total();
            $('#pago_efe').val($('.totalPedido').val());
            $('.opc-01').text(formatNumber($('.totalPedido').val()));
        } else if (result.dismiss === Swal.DismissReason.cancel) {
