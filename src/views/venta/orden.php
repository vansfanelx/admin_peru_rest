<script>
    var imp_val = <?php echo (float)Session::get('igv'); ?>;
</script>
<?php
                            ...
                            <table class="table table-condensed m-t-0 m-b-0" style="background: #fcf8ec; border-bottom-left-radius: .3rem;">
                                <thead>
                                    ...
                                    <tr class="text-muted font-13 display-comision-delivery">
                                     ...
                                     <tr class="text-muted font-13 display-comision-delivery">
                                        <th class="p-b-5"><a href="javascript:void(0)" class="link" onclick="propina_factura();">Propina <i class="ti-info-alt text-warning font-10" data-original-title="Puede ingresar un monto por el servicio aquí" data-toggle="tooltip" data-placement="top"></i></a></th>
                                        <th class="text-right p-b-5">
                                            <input type="hidden" name="propina" id="propina" class="propina" value="0.00" autocomplete="off">
                                            <span><?php echo Session::get('moneda'); ?>
                                            <span class="propina">0.00</span>
                                            </span>
                                        </th>
                                    </tr>
...
<?php } ?>
<script>
// Asegura que la propina se incluya en el objeto venta enviado al backend
$(document).ready(function() {
    // Intercepta el submit del formulario de facturación
    $('#form-facturar').on('submit', function(e) {
        // Antes de enviar, asegura que el valor de propina esté en el objeto venta
        var propina = $('#propina').val() || '0.00';
        // Busca el objeto venta en el contexto JS (usualmente se arma en venta_orden.js)
        if (typeof venta === 'object') {
            venta.propina = propina;
        } else {
            // Si no existe, lo agrega al form como input oculto (fallback)
            if ($('input[name="propina"]').length === 0) {
                $(this).append('<input type="hidden" name="propina" value="'+propina+'">');
            } else {
                $('input[name="propina"]').val(propina);
            }
        }
    });
});
