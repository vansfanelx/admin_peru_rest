...
// Inicializar variables para pagos mixtos
$pagos_mixtos = null;
$otros_pagos = [];
foreach($this->dato->Pagos as $d){ 
    if(stripos($d->DESCRIPCION, 'MIXTO') !== false || stripos($d->DESCRIPCION, 'EFECTIVO / TARJETA') !== false || stripos($d->DESCRIPCION, 'EFECTIVO/TARJETA') !== false) {
        $pagos_mixtos = $d;
    } else {
        $otros_pagos[] = $d;
    }
...
// Mostrar primero efectivo, tarjeta, etc.
foreach($otros_pagos as $d){ 
    $pdf->Cell(32, 4, $d->DESCRIPCION, 0);   
    $pdf->Cell(15, 4, $d->TRANSACCIONES,0,0,'R');
    $pdf->Cell(25, 4, number_format(($d->MONTO),2),0,0,'R');
    $pdf->Ln(4);
}
// Mostrar pagos mixtos si existen
if($pagos_mixtos) {
    $pdf->Cell(32, 4, 'MIXTO/TARJETA', 0);   
    $pdf->Cell(15, 4, $pagos_mixtos->TRANSACCIONES,0,0,'R');
    $pdf->Cell(25, 4, number_format(($pagos_mixtos->MONTO),2),0,0,'R');
    $pdf->Ln(4);
}

// Mostrar total de propinas si está disponible
if (isset($this->dato->total_propina)) {
    $pdf->SetFont('Courier','B',9);
    $pdf->Cell(32, 4, 'PROPINAS', 0);
    // Mostrar cantidad de operaciones con propina si está disponible
    $cantPropina = isset($this->dato->cant_propina) ? $this->dato->cant_propina : '';
    $pdf->Cell(15, 4, $cantPropina, 0, 0, 'R');
    $pdf->Cell(25, 4, number_format($this->dato->total_propina, 2), 0, 0, 'R');
    $pdf->Ln(4);
}
...
$pdf->Ln(4); 
//eliminar esta linea es debug
// $pdf->Cell(0, 4, 'DEBUG CORTESIAS: '.json_encode($this->dato->Cortesias), 0, 1);
if (isset($this->dato->Cortesias) && $this->dato->Cortesias->total > 0) {
    $pdf->Cell(32, 4, 'CORTESIAS', 0);
    $pdf->Cell(15, 4, $this->dato->Cortesias->cant, 0, 0, 'R');
    $pdf->Cell(25, 4, number_format($this->dato->Cortesias->total, 2), 0, 0, 'R');
    $pdf->Ln(8);
}
...
// PIE DE PAGINA
$pdf->Ln(10);
$pdf->Output('cierre_'.date('d-m-Y-H-i',strtotime($this->dato->fecha_registro)).'.pdf','i');
