...
class FPDF_CellFiti extends FPDF_CellFit
{
    ...
    $pdf->Cell(72,4,utf8_decode(Session::get('tribAcr') ? Session::get('tribAcr') : 'RUC').': '.utf8_decode($this->empresa['ruc']),0,1,'C');
    ...
    // DATOS FACTURA
    $elec = (($this->dato->id_tdoc == 1 || $this->dato->id_tdoc == 2) && Session::get('sunat') == 1) ? 'ELECTRONICA' : 'ELECTRONICA';     
    ...
    $pdf->Cell(72,4,utf8_decode(Session::get('diAcr') ? Session::get('diAcr') : 'DNI' ).': '.utf8_decode($this->dato->Cliente->dni),0,1,'');
    }else{
    $pdf->Cell(72,4,utf8_decode(Session::get('tribAcr')? Session::get('diAcr') : 'RUC' ).': '.utf8_decode($this->dato->Cliente->ruc),0,1,'');
    }
    ...
    */
    ...
    $pdf->Cell(65, 1, utf8_decode(Session::get('tribAcr') ? Session::get('tribAcr') : 'RUC').': '.utf8_decode($this->empresa['ruc']), 0, 0,'C');
    $pdf->Ln();
    ...
    $pdf->setY(71);
    $pdf->setX(10);
    $pdf->Cell(46.5, $heightc2, 'R.U.C / D.N.I:', 1, 0, 'C');

    if($this->dato->Cliente->tipo_cliente == 1){
        $numero_doc_cli = utf8_decode(Session::get('diAcr') ? Session::get('diAcr') : 'DNI' ).': '.utf8_decode($this->dato->Cliente->dni);
    }else{
        $numero_doc_cli = utf8_decode(Session::get('tribAcr')? Session::get('diAcr') : 'RUC' ).': '.utf8_decode($this->dato->Cliente->ruc);
    }
    ...
    $pdf->Cell(32, $heightc2, '', 0,'L');
    $pdf->Cell(70, $heightc2, 'https://www.sunat.gob.pe', 0, 1,'L');
    $pdf->Ln();
    ...
    //[Condicional SUB TOTAL E IGV]
    if($total_ope_exoneradas > 0){
        $sub_total = number_format(((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del)),2);
        $igv = number_format(((($this->dato->total- $this->dato->desc_monto) +$this->dato->comis_del) - ((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del))), 2);
    }else{
        //$sub_total = number_format(((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / 1.18),2);
        $sub_total = number_format(((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / (1 + $this->dato->igv)),2);
        //$igv = number_format(((($this->dato->total- $this->dato->desc_monto) +$this->dato->comis_del) - ((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / 1.18)), 2);
        $igv = number_format(((($this->dato->total- $this->dato->desc_monto) +$this->dato->comis_del) - ((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / (1 + $this->dato->igv))), 2);
    }
    ...
    if($total_ope_gravadas > 0){
        ...
        $op_gravada = number_format(((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / (1 + $this->dato->igv)),2); 
    }else{
    ...
    $data2 = array(
        ...
        array(Session::get('impAcr') ? Session::get('impAcr') . ' ('.($this->dato->igv * 100) .'%)' : 'IGV' , $igv)
    );
...
$pdf->Ln(4); 
$pdf->Cell(37, 10,Session::get('impAcr') ? Session::get('impAcr') : 'IGV', 0);    
$pdf->Cell(20, 10, '', 0);
($total_ope_exoneradas > 0 )?
