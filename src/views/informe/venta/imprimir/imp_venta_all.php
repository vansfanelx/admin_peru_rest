...
$pdf->Cell(15, 10, number_format(((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / (1 + $this->dato->igv)),2),0,0,'R');	
...
$pdf->Ln(4); 
$pdf->Cell(37, 10,Session::get('impAcr') ? Session::get('impAcr') . ' ('.($this->dato->igv * 100) .'%)' : 'IGV' . ' ('.($this->dato->igv * 100) .'%)', 0);    
$pdf->Cell(20, 10, '', 0);
($total_ope_exoneradas > 0 )?
...
$pdf->Cell(15, 10, number_format(((($this->dato->total- $this->dato->desc_monto) +$this->dato->comis_del) - ((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / (1 + $this->dato->igv))),2),0,0,'R');
...
if(($this->dato->id_tdoc == 1 || $this->dato->id_tdoc == 2)){
	...
	$igv_qr = number_format(((($this->dato->total- $this->dato->desc_monto) + $this->dato->comis_del) - ((($this->dato->total - $this->dato->desc_monto) +$this->dato->comis_del) / (1 + $this->dato->igv))),2);
	//number_format(($total_igv_gravadas + $total_igv_exoneradas),2)
    
	$nombreqr=$tc."-".$td.$this->dato->ser_doc."-".$this->dato->nro_doc;
	$text_qr = $this->empresa['ruc'] . '|' . $tc . '|' . $this->dato->ser_doc . '|' . $this->dato->nro_doc .'|'.$igv_qr.'|'.number_format(($this->dato->total),2).'|'.date('Y-m-d',strtotime($this->dato->fec_ven)).'|'. $tdd . '|' . $tdc . '|'.$this->dato->hash_cdr;
...
//$igv_qr  . '--' . number_format(($total_igv_gravadas + $total_igv_exoneradas),2) . 'igv g:' . $total_igv_gravadas . 'igv o:' . $total_igv_exoneradas . 
